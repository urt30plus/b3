...
class FlagstatsPlugin(b3.plugin.Plugin):
    ...
    def update_hall_of_fame(
        self, blue_team: TeamData, red_team: TeamData, map_name: str
    ) -> None:
        ...
        if action in self.FLAG_TAKEN_ACTIONS and other_team.flag_taken == 0:
            ...
            if client:
                ...
                if curr_best.value == -1 or curr_best.value > time_capture:
                    ...
                    self.show_message_to_client(
                        client, time_capture, best_time=show_best_time
                    )
                else:
    ...
    def show_message_to_client(self, client, time_capture, best_time=False):
        ...
        self.console.write(f"{client.name}^3 captured the flag in ^5{cap_time}^3")
    ...
    def cmd_flagrecord(self, data, client, cmd=None):
        ...
        """
        try:
            caps_record = b3.plugins.hof.record_holder(self.console, "flagstats_caps")
            time_record = b3.plugins.hof.record_holder(self.console, "flagstats_time")
        except LookupError:
