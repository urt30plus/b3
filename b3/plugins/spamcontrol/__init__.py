...
class SpamcontrolPlugin(b3.plugin.Plugin):
    ...
    def onLoadConfig(self):
        ...
        """
        self._maxSpamins = self.getSetting(
            "settings",
            "max_spamins",
            b3.INTEGER,
            self._maxSpamins,
        ...
        self._modLevel = self.getSetting(
            "settings", "mod_level", b3.LEVEL, self._modLevel
        )
        self._falloffRate = self.getSetting(
            "settings", "falloff_rate", b3.FLOAT, self._falloffRate
        )
    ...
    def cmd_spamins(self, data, client, cmd=None):
        ...
        if sclient.maxLevel >= self._modLevel:
            ...
            cmd.sayLoudOrPM(
                client,
                "%s ^7currently has %s spamins, peak was %s"
                % (sclient.exactName, smin, msmin),
            )
