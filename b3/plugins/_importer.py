...
def _import_plugin(console, name: str, path: str = None) -> types.ModuleType:
    ...
    try:
        ...
        if ext_plugin_dir not in sys.path:
            console.warning(
                "Appending external directory %s to sys.path", ext_plugin_dir
            )
...
def _search_config_file(match: str, ext_plugins_dir: str = None) -> list[str]:
    ...
    if not collection and ext_plugins_dir:
        ext_conf_path = os.path.join(
            b3.functions.getAbsolutePath(ext_plugins_dir, decode=True), match, "../conf"
        )
...
def _get_plugin_config(
    ...
    if p_config_path is None:
        ...
        console.warning(
            "No configuration file specified for plugin %s: "
            "searching a valid configuration file...",
            p_name,
        )
        ...
        console.bot(
            "Loading configuration file %s for plugin %s", search_path[0], p_name
        )
        ...
        p_config_absolute_path = b3.functions.getAbsolutePath(
            p_config_path, decode=True
        )
        if os.path.exists(p_config_absolute_path):
            console.bot(
                "Loading configuration file %s for plugin %s",
                p_config_absolute_path,
                p_name,
            )
...
def _check_plugin_version(p_data: PluginData) -> None:
    if p_data.clazz.requiresVersion and B3version(
        p_data.clazz.requiresVersion
    ) > B3version(b3_version):
...
def _check_plugin_game(p_data: PluginData, game_name: str) -> None:
    if p_data.clazz.requiresParsers and game_name not in p_data.clazz.requiresParsers:
        raise b3.config.MissingRequirement(
...
def _check_plugin_storage(p_data: PluginData, storage_proto: str) -> None:
    if (
        ...
        and storage_proto not in p_data.clazz.requiresStorage
    ):
        raise b3.config.MissingRequirement(
        ...
        if p_data.clazz.requiresPlugins:
            for r in p_data.clazz.requiresPlugins:
                if r not in all_plugins:
                    raise b3.config.MissingRequirement(f"missing required plugin: {r}")
...
def _sort_plugins(console, plugin_data: dict[str, PluginData]) -> list:
    ...
    sorted_plugin_list += [
        plugin_data[x] for x in graphlib.TopologicalSorter(plugin_graph).static_order()
    ]
...
def _create_plugin_instances(console, plugins: list[PluginData]) -> None:
    console.bot(
        "Ready to create plugin instances: %s", ", ".join([x.name for x in plugins])
    )
    ...
    for plugin_data in plugins:
        ...
        try:
            ...
            console.error("Could not load plugin %s", plugin_data.name, exc_info=err)
            ...
            console.bot("Plugin %s (%s - %s) loaded", plugin_data.name, version, author)
