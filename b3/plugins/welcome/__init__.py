...
class WelcomePlugin(b3.plugin.Plugin):
    ...
    def _load_config_flags(self):
        ...
        config_options = list(zip(*flag_options, strict=True))[0]
            ...
            for opt, F in flag_options:
                if self.config.has_option("settings", opt):
                    try:
                        ...
                        self.error(
                            "could not load settings/%s config value: %s", opt, e
                        )
                else:
                    set_flag(F)
    ...
    def onAuth(self, event):
        if (
            self._welcomeFlags <= 0
            or not event.client
            or event.client.id is None
            or event.client.cid is None
            or not event.client.connected
            or event.client.pbid == "WORLD"
        ):
            return
    ...
    def welcome(self, client):
        ...
        if client.connected and _timeDiff > self._min_gap:
            ...
            if client.connections >= 2:
                ...
                if (
                    self._welcomeFlags & F_ANNOUNCE_USER
                    and client.connections < self._newbConnections
                ):
    ...
    def cmd_greeting(self, data, client, cmd=None):
        ...
        if data.lower() == "none":
            ...
            prepared_data = re.sub(
                r"\$(name|maxLevel|group|connections)", r"%(\1)s", data
            )
