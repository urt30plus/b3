...
class StorageAPITest:
    ...
    def test_setClientAlias(self):
        ...
        alias_id3 = self.storage.setClientAlias(
            Alias(id=alias_id1, alias="billy", clientId=1)
        )
    ...
    def test_setClientIpAddress(self):
        ...
        ipalias_id2 = self.storage.setClientIpAddress(
            IpAlias(ip="127.0.0.1", clientId=1)
        )
    ...
    def test_getClientIpAddress(self):
        ipalias = IpAlias(
            ip="88.44.55.22", timeAdd=12, timeEdit=654, numUsed=7, clientId=54
        )
        ...
        ipalias2 = self.storage.getClientIpAddress(
            IpAlias(clientId=ipalias.clientId, ip=ipalias.ip)
        )
    ...
    def test_getLastPenalties(self):
        ...
        c2.id = 18
        Penalty(
            clientId=c1.id, adminId=0, inactive=1, type="Ban", timeExpire=-1, data="pA"
        ).save(self.console)
        Penalty(
            clientId=c1.id,
            adminId=0,
            inactive=0,
            type="Ban",
            timeExpire=self.console.time() + 10,
            data="pB",
        ).save(self.console)
        Penalty(
            clientId=c1.id,
        ...
        Penalty(
            clientId=c2.id, adminId=0, inactive=0, type="Ban", timeExpire=-1, data="pH"
        ).save(self.console)
        ...
        def getLastPenalties(types, num):
            ...
            for p in self.storage.getLastPenalties(types=types, num=num):
                ...
                self.assertTrue(
                    p.timeExpire == -1 or p.timeExpire > self.console.time()
                )
        ...
        self.assertListEqual(
            ["pH", "pG", "pF", "pC", "pB"],
            getLastPenalties(
                types=("Ban", "TempBan", "Kick", "Warning", "Notice"), num=5
            ),
        )
        self.assertListEqual(
            ["pH", "pG", "pF", "pC"],
            getLastPenalties(
                types=("Ban", "TempBan", "Kick", "Warning", "Notice"), num=4
            ),
        )
        self.assertListEqual(
            ["pH", "pG", "pB"], getLastPenalties(types=("Ban", "TempBan"), num=5)
        )
    ...
    def test_setClientPenalty(self):
        ...
        id3 = self.storage.setClientPenalty(
            Penalty(id=id1, type="Warning", clientId=1, adminId=0)
        )
        self.assertEqual(id1, id3)

    def test_setClientPenalty_bad_keyword(self):
        id1 = self.storage.setClientPenalty(
            Penalty(keyword="!=+", clientId=1, adminId=0)
        )
        self.assertIsNotNone(id1)
    ...
    def test_getClientLastPenalty(self):
        ...
        penalty = self.storage.getClientLastPenalty(
            client=client, type=("Ban", "Kick", "Notice")
        )
        self.assertIsInstance(penalty, Penalty)
        self.assertEqual(penalty.data, "pE")
    ...
    def test_getClientFirstPenalty(self):
        ...
        penalty = self.storage.getClientFirstPenalty(
            client=client, type=("Ban", "Kick")
        )
        self.assertIsInstance(penalty, Penalty)
        self.assertEqual(penalty.data, "pA")
        penalty = self.storage.getClientFirstPenalty(
    ...
    def test_disableClientPenalties(self):
        ...
        c2.id = 18
        Penalty(
            clientId=c1.id, adminId=0, timeExpire=-1, type="Ban", inactive=1, data="pA"
        ).save(self.console)
        Penalty(
            clientId=c1.id, adminId=0, timeExpire=-1, type="Ban", inactive=0, data="pB"
        ).save(self.console)
        Penalty(
            clientId=c1.id,
            adminId=0,
        ...
        Penalty(
            clientId=c1.id, adminId=0, timeExpire=-1, type="Kick", inactive=0, data="pD"
        ).save(self.console)
        Penalty(
            clientId=c2.id,
            adminId=0,
        ...
        assertPenalties(
            client=c1,
            types=("Ban", "TempBan", "Kick", "Warning", "Notice"),
            penalties_in=("pB", "pC", "pD", "pG"),
            penalties_notin=("pA", "pE", "pF"),
        )
        assertPenalties(
            client=c2,
            types=("Ban", "TempBan", "Kick", "Warning", "Notice"),
            penalties_in=("pE", "pF"),
            penalties_notin=("pA", "pB", "pC", "pD", "pG"),
        )
        ...
        assertPenalties(
            client=c1,
            types=("Ban", "TempBan", "Kick", "Warning", "Notice"),
            penalties_in=("pC", "pD", "pG"),
            penalties_notin=("pA", "pB", "pE", "pF"),
        )
        assertPenalties(
            client=c2,
            types=("Ban", "TempBan", "Kick", "Warning", "Notice"),
        ...
        assertPenalties(
            ...
            types=("Ban", "TempBan", "Kick", "Warning", "Notice"),
            penalties_in=("pC", "pG"),
            penalties_notin=("pA", "pB", "pD", "pE", "pF"),
        )
        assertPenalties(
            client=c2,
            types=("Ban", "TempBan", "Kick", "Warning", "Notice"),
    ...
    def test_numPenalties(self):
        ...
        self.assertEqual(
            4,
            self.storage.numPenalties(
                client=c1, type=("Ban", "TempBan", "Kick", "Warning", "Notice")
            ),
        )
        ...
        self.assertEqual(
            2,
            self.storage.numPenalties(
                client=c2, type=("Ban", "TempBan", "Kick", "Warning", "Notice")
            ),
        )
    ...
    def test_getCounts(self):
        ...
        self.assertEqual(
            {"Kicks": 0, "TempBans": 0, "clients": 2, "Bans": 1, "Warnings": 0},
            self.storage.getCounts(),
        )
        Penalty(clientId=c1.id, adminId=0, type="Ban").save(self.console)
        self.assertEqual(
            {"Kicks": 0, "TempBans": 0, "clients": 2, "Bans": 2, "Warnings": 0},
            self.storage.getCounts(),
        )
        Penalty(clientId=c1.id, adminId=0, type="TempBan").save(self.console)
        self.assertEqual(
            {"Kicks": 0, "TempBans": 1, "clients": 2, "Bans": 2, "Warnings": 0},
            self.storage.getCounts(),
        )
        Penalty(clientId=c1.id, adminId=0, type="Kick").save(self.console)
        self.assertEqual(
            {"Kicks": 1, "TempBans": 1, "clients": 2, "Bans": 2, "Warnings": 0},
            self.storage.getCounts(),
        )
        Penalty(clientId=c1.id, adminId=0, type="Warning").save(self.console)
        self.assertEqual(
            {"Kicks": 1, "TempBans": 1, "clients": 2, "Bans": 2, "Warnings": 1},
            self.storage.getCounts(),
        )
    ...
    def test_truncateTables_invalid_table_name(self):
        ...
        self.assertRaises(
            KeyError, self.storage.truncateTable, ["invalid_table1", ["invalid_table2"]]
        )
