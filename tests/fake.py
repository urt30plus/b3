...
import b3.output
import b3.parser
from b3.clients import Clients, Cvar
...
class FakeConsole(b3.parser.Parser):
    ...
    def __init__(self, config):
        ...
        logging.basicConfig(
            level=b3.output.VERBOSE2, format="%(asctime)s\t%(levelname)s\t%(message)s"
        )
    ...
    def queueEvent(self, event, expire=10):
        ...
        if not hasattr(event, "type"):
            ...
            self.verbose(
                "Queueing event %s %s", self.Events.getName(event.type), event.data
            )
    ...
    def _handleEvent(self, event):
        ...
        NO QUEUE, NO THREAD for faking speed up
        """
        if event.type == self.getEventID("EVT_EXIT") or event.type == self.getEventID(
            "EVT_STOP"
        ):
            self.working = False
        ...
        for hfunc in self._handlers[event.type]:
            ...
            self.verbose(
                "parsing event: %s: %s",
                self.Events.getName(event.type),
            ...
            try:
                ...
                self.bot(
                    "Event %s vetoed by %s", self.Events.getName(event.type), str(hfunc)
                )
                break
                ...
                self.error(
                    "handler %s could not handle event %s: %s: %s %s",
                    hfunc.__class__.__name__,
                    self.Events.getName(event.type),
    ...
    def kick(self, client, reason="", admin=None, silent=False, *kwargs):
        ...
        self.queueEvent(
            self.getEvent(
                "EVT_CLIENT_KICK",
                data={"reason": reason, "admin": admin},
                client=client,
            )
        )
        client.disconnect()
    ...
    def message(self, client, text, *args):
        ...
        if client is None:
            ...
            print(
                "sending msg to %s: %s"
                % (client.name, re.sub(re.compile(r"\^[0-9]"), "", text % args).strip())
            )
...
class FakeClient(b3.clients.Client):
    ...
    def connects(self, cid):
        ...
        self.console.debug(
            "client connected: [%s] %s - %s (%s)",
            clients[self.cid].cid,
            clients[self.cid].name,
            clients[self.cid].guid,
            clients[self.cid].data,
        )

        self.console.queueEvent(
            self.console.getEvent("EVT_CLIENT_CONNECT", data=self, client=self)
        )
    ...
    def says(self, msg):
        print(f'\n{self.name} says "{msg}"')
        self.console.queueEvent(
            self.console.getEvent("EVT_CLIENT_SAY", data=msg, client=self)
        )

    def says2team(self, msg):
        print(f'\n{self.name} says to team "{msg}"')
        self.console.queueEvent(
            self.console.getEvent("EVT_CLIENT_TEAM_SAY", data=msg, client=self)
        )

    def says2squad(self, msg):
        print(f'\n{self.name} says to squad "{msg}"')
        self.console.queueEvent(
            self.console.getEvent("EVT_CLIENT_SQUAD_SAY", data=msg, client=self)
        )
    ...
    def says2private(self, msg):
        ...
        self.console.queueEvent(
            self.console.getEvent(
                "EVT_CLIENT_PRIVATE_SAY", data=msg, client=self, target=self
            )
        )
    ...
    def damages(self, victim, points=34.0):
        ...
        data = (points, 1, 1, 1)
        self.console.queueEvent(
            self.console.getEvent(eventkey, data=data, client=self, target=victim)
        )
    ...
    def suicides(self):
        ...
        self.console.queueEvent(
            self.console.getEvent(
                "EVT_CLIENT_SUICIDE", data=data, client=self, target=self
            )
        )

    def do_action(self, actiontype):
        self.console.queueEvent(
            self.console.getEvent("EVT_CLIENT_ACTION", data=actiontype, client=self)
        )
