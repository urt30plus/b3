...
class Test_misc_cmd(Admin_TestCase):
    ...
    def test_maps(self):
        ...
        self.p.cmd_maps(data=None, client=mock_client, cmd=mock_cmd)
        mock_cmd.sayLoudOrPM.assert_called_once_with(
            mock_client, "^7Map Rotation list is empty"
        )
        ...
        self.p.cmd_maps(data=None, client=mock_client, cmd=mock_cmd)
        mock_cmd.sayLoudOrPM.assert_called_once_with(
            mock_client, "^7Map Rotation: ^2foo"
        )
        ...
        self.p.cmd_maps(data=None, client=mock_client, cmd=mock_cmd)
        mock_cmd.sayLoudOrPM.assert_called_once_with(
            mock_client, "^7Map Rotation: ^2foo1^7, ^2foo2^7, ^2foo3"
        )
...
class Test_cmd_iamgod(CommandTestCase):
    ...
    def iamgod(self, data=""):
        return self.p.cmd_iamgod(
            data=data, client=self.mock_client, cmd=self.mock_command
        )

    def test_when_there_is_already_a_superadmin(self):
        self.p._commands["iamgod"] = "foo"
        self.p.warning = Mock()
        self.p.console.clients.lookupSuperAdmins = Mock(
            return_value=[Mock(spec=Client)]
        )
    ...
    def test_is_already_superadmin(self):
        ...
        self.mock_client.message.assert_called_once_with(
            "^7You are already a ^2superadmin"
        )
    ...
    def test_when_there_is_no_superadmin(self):
        ...
        self.mock_client.message.assert_called_once_with(
            "^7You are now a ^2%s" % mock_superadmin_group.name
        )
...
class Test_cmd_warn(CommandTestCase):
    ...
    def test_prevent_warn_self(self):
        ...
        self.warn("foo")
        self.p.getMessage.assert_called_once_with(
            "warn_self", self.mock_client.exactName
        )
        assert not self.p.warnClient.called
    ...
    def test_player_is_higher_level(self):
        ...
        self.warn("foo")
        self.p.getMessage.assert_called_once_with(
            "warn_denied", self.mock_client.exactName, foo_player.exactName
        )
        assert not self.p.warnClient.called
    ...
    def test_already_warned_recently(self):
        ...
        self.warn("foo")
        self.mock_client.message.assert_called_once_with(
            "^7Only one warning per 15 seconds can be issued"
        )
        assert not self.p.warnClient.called
    ...
    def test_nominal_with_keyword(self):
        ...
        self.p.warnClient.assert_called_once_with(
            foo_player, "thekeyword", self.mock_client
        )
...
class Test_cmd_kick(CommandTestCase):
    ...
    def test_no_reason(self):
        ...
        self.kick("foo")
        self.mock_client.message.assert_called_once_with(
            "^1ERROR: ^7You must supply a reason"
        )
        assert not self.mock_client.kick.called
    ...
    def test_prevent_kick_self(self):
        ...
        self.kick("foo")
        self.p.getMessage.assert_called_once_with(
            "kick_self", self.mock_client.exactName
        )
        assert not self.mock_client.kick.called
    ...
    def test_player_is_higher_level_but_masked(self):
        ...
        self.kick("foo")
        self.mock_client.message.assert_called_once_with(
            "^7%s ^7is a masked higher level player, action cancelled"
            % foo_player.exactName
        )
        assert not self.mock_client.kick.called
    ...
    def test_nominal_with_reason(self):
        ...
        foo_player.kick.assert_called_once_with(
            "theReason", "theReason", self.mock_client
        )
    ...
    def test_nominal_with_reason_keyword(self):
        ...
        foo_player.kick.assert_called_once_with(
            "aReason", "theKeyword", self.mock_client
        )
...
class Test_cmd_spank(CommandTestCase):
    ...
    def test_nominal_with_reason(self):
        ...
        foo_player.kick.assert_called_once_with(
            "theReason", "theReason", self.mock_client, silent=True
        )
    ...
    def test_nominal_with_reason_keyword(self):
        ...
        foo_player.kick.assert_called_once_with(
            "aReason", "theKeyword", self.mock_client, silent=True
        )
...
class Test_cmd_permban(CommandTestCase):
    ...
    def test_prevent_permban_self(self):
        ...
        self.permban("foo")
        self.p.getMessage.assert_called_once_with(
            "ban_self", self.mock_client.exactName
        )
        assert not self.mock_client.ban.called
    ...
    def test_player_is_higher_level(self):
        ...
        self.permban("foo")
        self.p.getMessage.assert_called_once_with(
            "ban_denied", self.mock_client.exactName, foo_player.exactName
        )
        assert not self.mock_client.ban.called
    ...
    def test_nominal_with_reason(self):
        ...
        foo_player.ban.assert_called_once_with(
            "theReason", "theReason", self.mock_client
        )
...
class Test_cmd_tempban(CommandTestCase):
    ...
    def test_prevent_tempban_self(self):
        ...
        self.tempban("foo 3h")
        self.p.getMessage.assert_called_once_with(
            "temp_ban_self", self.mock_client.exactName
        )
        assert not self.mock_client.tempban.called
    ...
    def test_player_is_higher_level(self):
        ...
        self.tempban("foo 3h")
        self.p.getMessage.assert_called_once_with(
            "temp_ban_denied", self.mock_client.exactName, foo_player.exactName
        )
        assert not self.mock_client.tempban.called
    ...
    def test_nominal_with_reason(self):
        ...
        foo_player.tempban.assert_called_once_with(
            "theReason", "theReason", 3 * 60, self.mock_client
        )
    ...
    def test_nominal_with_reason_keyword(self):
        ...
        foo_player.tempban.assert_called_once_with(
            "aReason", "theKeyword", 3 * 60, self.mock_client
        )
...
"""
    ...
    def test_too_many_rules(self, sleep_mock):
        ...
        self.p._sendRules(None)
        self.console.say.assert_has_calls(
            [call("this is rule #%s" % x) for x in range(1, 20)]
        )
...
class Test_cmd_lastbans(CommandTestCase):
    ...
    def test_no_ban(self):
        ...
        self.mock_command.sayLoudOrPM.assert_called_once_with(
            self.player, "^7There are no active bans"
        )
    ...
    def test_one_ban(self):
        ...
        when(self.console.storage).getLastPenalties(
            types=whatever(), num=whatever()
        ).thenReturn([penalty1])
        # WHEN
        self.lastbans()
        # THEN
        self.mock_command.sayLoudOrPM.assert_called_once_with(
            self.player, "^2@1^7 Bill^7^7 (Perm)"
        )
    ...
    def test_one_ban_with_reason(self):
        ...
        player1.save()
        penalty1 = ClientBan(
            clientId=player1.id, timeExpire=-1, adminId=0, reason="test reason"
        )
        when(self.console.storage).getLastPenalties(
            types=whatever(), num=whatever()
        ).thenReturn([penalty1])
        ...
        # THEN
        self.mock_command.sayLoudOrPM.assert_called_once_with(
            self.player, "^2@1^7 Bill^7^7 (Perm) test reason"
        )
    ...
    def test_two_bans_with_reason(self):
        ...
        self.mock_command.sayLoudOrPM.assert_has_calls(
            [
                ...
                call(
                    self.player, "^2@2^7 P2^7^7 (2 minutes remaining) test reason f00"
                ),
            ]
        )
...
"""
    ...
    def test_non_existing_player(self):
        ...
        self.assertListEqual(
            ["No players found matching f00"], self.joe.message_history
        )
        self.assertEqual("user", self.mike.maxGroup.keyword)
    ...
    def test_already_in_group(self):
        ...
        # THEN
        self.assertListEqual(
            ["Mike is already in group User"], self.joe.message_history
        )
        self.assertEqual("user", self.mike.maxGroup.keyword)
    ...
    def test_group_beyond_reach(self):
        # GIVEN
        jack = FakeClient(
            self.console, name="Jack", guid="jackguid", groupBits=40, team=TEAM_RED
        )
        jack.connects(3)
        ...
        self.assertEqual("user", self.mike.maxGroup.keyword)
        self.assertListEqual(
            ["You can't assign players to group Full Admin"], jack.message_history
        )
        # WHEN
        jack.clearMessageHistory()
...
class Test_cmd_lastbans_functional(Admin_functional_test):
    ...
    def test_one_tempban(self, mock_time):
        ...
        self.joe.message.assert_called_with(
            "^2@2^7 Mike^7^7 (5 hours remaining) test reason"
        )
        # WHEN
...
class Test_cmd_help(Admin_functional_test):
    ...
    def test_existing_cmd(self):
        ...
        self.joe.message.assert_called_with(
            "^2!help ^7%s" % self.p.cmd_help.__doc__.strip()
        )

    def test_no_arg(self):
        self.joe.says("!help")
        self.joe.message.assert_called_with(
            "^7Available commands: admins, admintest, aliases, b3, ban, banall, baninfo"
...
class Test_cmd_mask_functional(Admin_functional_test):
    ...
    def test_nominal(self):
        ...
        self.joe.says("!admins")
        self.joe.message.assert_called_with(
            "^7Admins online: Joe^7^7 [^3100^7], Mike^7^7 [^380^7]"
        )
    ...
    def _test_persistence_for_group(self, group_keyword):
        ...
        self.assertEqual(
            group_to_mask_as.id,
            self.joe.maskGroup.id,
            "expecting Joe2 to have %s for the mask group id" % group_to_mask_as.id,
        )
        self.assertEqual(
            group_to_mask_as.level,
            self.joe.maskGroup.level,
            "expecting Joe2 to have %s for the mask group level"
            % group_to_mask_as.level,
        )
        self.assertEqual(
            group_to_mask_as.id,
            self.joe.maskedGroup.id,
            "expecting Joe2 to have %s for the masked group id" % group_to_mask_as.id,
        )
        self.assertEqual(
            group_to_mask_as.level,
            self.joe.maskedGroup.level,
            "expecting Joe2 to have %s for the masked group level"
            % group_to_mask_as.level,
        )
        ## WHEN joe reconnects
        ...
        self.assertEqual(
            group_to_mask_as.level,
            client_data._maskLevel,
            "expecting %s to be the value in the mask_level column in database"
            % group_to_mask_as.level,
        )
...
class Test_cmd_rules(Admin_functional_test):
    ...
    def test_nominal(self, start_new_thread_mock, sleep_mock):
        ...
        self.joe.connects(0)
        self.joe.says("!rules")
        self.joe.message.assert_has_calls(
            [
                call("^3Rule #1: No racism of any kind"),
                call(
    ...
    def test_nominal_bigtext(self, start_new_thread_mock, sleep_mock):
        ...
        self.joe.connects(0)
        self.joe.says("&rules")
        self.console.saybig.assert_has_calls(
            [
                call("^3Rule #1: No racism of any kind"),
                call(
                    "^3Rule #2: No clan stacking, members must split evenly between the teams"
    ...
    def test_nominal_to_player(self, start_new_thread_mock, sleep_mock):
        ...
        self.mike.message.assert_has_calls(
            [
                call("^3Rule #1: No racism of any kind"),
                call(
                    "^3Rule #2: No clan stacking, members must split evenly between the teams"
...
class Test_cmd_warns(Admin_functional_test):
    ...
    def test_nominal(self):
        ...
        self.joe.message.assert_called_once_with(
            "^7Warnings: adv, afk, argue, badname, camp, ci, color, cuss, fakecmd,"
...
"""
    ...
    def test_two_admins(self):
        ...
        # THEN
        self.assertListEqual(
            ["Admins online: Joe [100], Mike [80]"], self.joe.message_history
        )
...
class Test_cmd_map(Admin_functional_test):
    ...
    def test_suggestions(self):
        ...
        self.joe.message.assert_called_once_with(
            "do you mean: bar1, bar2, bar3, bar4, bar5?"
        )
...
class Test_cmd_register(Admin_functional_test):
    def setUp(self):
        ...
        self.player = FakeClient(
            self.console, name="TestPlayer", guid="player_guid", groupBits=0
        )
...
"""
        ...
        # THEN
        self.assertListEqual(
            ["Thanks for your registration. You are now a member of the group User"],
            self.player.message_history,
        )
        self.assertListEqual(
            [call("TestPlayer^7 put in group User")], self.say_mock.mock_calls
        )
...
"""
        ...
        # THEN
        self.assertListEqual(
            ["You are now a member of the group User"], self.player.message_history
        )
        self.assertListEqual(
            [call("TestPlayer^7 is now a member of group User")],
...
"""
        ...
        # THEN
        self.assertListEqual(
            ["Spamages: bar, foo, rule1, rule2"], self.joe.message_history
        )
...
"""
    ...
    def test_warn(self, sleep_mock):
        ...
        self.assertListEqual(
            [call(60.0, "^7behave yourself", None, self.joe, "")],
            self.mike_warn_mock.mock_calls,
        )
        self.assertListEqual(
    ...
    def test_warn_then_auto_kick(self, instant_timer, sleep_mock):
        ...
        self.assertListEqual(
            [
                ...
                call(
                    "^1ALERT^7: Mike^7^7 auto-kick from warnings if not cleared [^33^7] ^7behave yourself"
                ),
            ],
            self.say_mock.mock_calls,
        )
        ...
        self.assertListEqual(
            [
                call(
                    "^7too many warnings: ^7behave yourself",
                    "None",
                    6,
                    self.joe,
                    False,
    ...
    def test_warn_self(self, sleep_mock):
        ...
        self.assertListEqual(
            ["Joe, you cannot give yourself a warning"], self.joe.message_history
        )
    ...
    def test_warn_denied(self, sleep_mock):
        ...
        self.assertEqual(0, self.joe.numWarnings)
        self.assertListEqual(
            ["Mike, Joe is a higher level admin, you can't warn him"],
    ...
    def test_clear_player(self, sleep_mock):
        ...
        self.assertListEqual(
            [
                call("^1WARNING^7 [^31^7]: Mike^7^7, ^7behave yourself"),
                call("Joe^7 has cleared Mike^7 of all warnings"),
            ],
            self.say_mock.mock_calls,
        )
        self.assertEqual(0, self.mike.numWarnings)
    ...
    def test_clear__all_players(self, sleep_mock):
        ...
        self.assertListEqual(
            [
                call("^1WARNING^7 [^31^7]: Mike^7^7, ^7behave yourself"),
                call("Joe^7 has cleared everyone's warnings and tk points"),
            ],
            self.say_mock.mock_calls,
        )
        self.assertEqual(0, self.mike.numWarnings)
...
class Test_warn_command_abusers(Admin_functional_test):
    def setUp(self):
        Admin_functional_test.setUp(self)
        self.player = FakeClient(
            self.console, name="ThePlayer", guid="theplayerguid", groupBits=0
        )
...
"""
        ...
        self.assertTupleEqual(
            (60.0, "^7do not use fake commands"), self.p.getWarning("fakecmd")
        )
...
"""
        ...
        self.assertEqual(
            call(
                "ThePlayer does not have sufficient rights to use !help. Required level: 2"
            ),
            info_mock.mock_calls[1],
        )
        # message section not loaded so this will fallback on the default warn message
        self.assertListEqual(
            ["You do not have sufficient access to use !help"],
            self.player.message_history,
        )
...
"""
        ...
        self.assertListEqual(
            [
                call(
                    1.5,
                    "do not use commands you do not have access to, try using !help",
                    "nocmd",
                    ANY,
                    ANY,
                )
...
"""
        ...
        self.player.connects("0")
        self.player.setvar(
            self.p, "fakeCommand", 2
        )  # simulate already 2 use of the !help command
        # WHEN
        self.player.says("!hzlp")
        self.player.says("!hzlp")
        self.player.says("!hzlp")
        ...
        self.assertListEqual(
            [call(120.0, "do not use fake commands", "fakecmd", ANY, ANY)],
            self.player_warn_mock.mock_calls,
        )
...
"""
    ...
    def test_normal_chat(self, sleep_mock):
        ...
        # THEN
        self.assertListEqual(
            ["Available commands: help, iamgod"], self.joe.message_history
        )
...
class Test_cmd_kick_functional(Admin_functional_test):
    ...
    def test_self_kick(self):
        ...
        self.assertListEqual([], self.kick_mock.mock_calls)
        self.assertListEqual(
            [call("^7Joe^7 ^7Can't kick yourself newb!")], say_mock.mock_calls
        )
    ...
    def test_no_reason_when_required(self):
        ...
        # THEN
        self.assertListEqual(
            ["You need to be in group Admin to use !kick"], self.joe.message_history
        )
        self.assertListEqual([], self.kick_mock.mock_calls)
    ...
    def test_kick_higher_level_admin(self):
        ...
        self.assertListEqual(
            [
                call(
                    "^7Joe^7^7 gets 1 point, Mike^7^7 gets none, Joe^7^7 wins, can't kick"
                )
            ],
            say_mock.mock_calls,
        )
    ...
    def test_kick_masked_higher_level_admin(self):
        ...
        # THEN
        self.assertListEqual(
            ["Joe is a masked higher level player, action cancelled"],
            self.mike.message_history,
        )
        self.assertListEqual([], self.kick_mock.mock_calls)
    ...
    def test_existing_player_name(self):
        ...
        # THEN
        self.assertListEqual(
            [call(self.mike, "the reason", self.joe, False)], self.kick_mock.mock_calls
        )
    ...
    def test_kick_by_slot_id(self):
        ...
        # THEN
        self.assertListEqual(
            [call(self.mike, "", self.joe, False)], self.kick_mock.mock_calls
        )
        self.assertEqual(1, self.console.storage.numPenalties(self.mike, "Kick"))
    ...
    def test_kick_by_database_id(self):
        ...
        # THEN
        self.assertListEqual(
            [call(ANY, "", self.joe, False)], self.kick_mock.mock_calls
        )
...
class Test_cmd_spam(Admin_functional_test):
    ...
    def test_unknown_spammage_keyword(self):
        ...
        self.assertListEqual([], say_mock.mock_calls)
        self.assertListEqual(
            ["Could not find spam message f00"], self.joe.message_history
        )
    ...
    def test_nominal_to_all_players(self):
        ...
        self.assertListEqual(
            [call("^3Rule #1: No racism of any kind")], say_mock.mock_calls
        )
        self.assertListEqual([], self.joe.message_history)
    ...
    def test_nominal_to_mike(self):
        ...
        self.assertListEqual([], self.joe.message_history)
        self.assertListEqual(
            ["Rule #1: No racism of any kind"], self.mike.message_history
        )
    ...
    def test_nominal_to_all_players_big(self):
        with patch.object(self.console, "saybig") as saybig_mock:
            self.joe.says("&spam rule1")
        self.assertListEqual(
            [call("^3Rule #1: No racism of any kind")], saybig_mock.mock_calls
        )
        self.assertListEqual([], self.joe.message_history)
