...
class Test_get_config_for_levels(Test_Tk_plugin):
    ...
    def test_level_junk(self):
        ...
        self.assertListEqual(
            [call("'f00' is not a valid level")], self.error_mock.mock_calls
        )
    ...
    def test_missing_section(self):
        ...
        # THEN
        self.assertListEqual(
            [call("section 'level_mod' is missing from the config file")],
    ...
    def test_bad_kill_multiplier(self):
        ...
        self.assertListEqual(
            [
                call(
                    "value for kill_multiplier is invalid. "
                    "could not convert string to float: {}".format("'f00'")
                ),
            ],
            self.error_mock.mock_calls,
        )
    ...
    def test_bad_damage_multiplier(self):
        ...
        self.assertListEqual(
            [
                call(
                    "value for damage_multiplier is invalid. "
                    "could not convert string to float: {}".format("'f00'")
                ),
            ],
            self.error_mock.mock_calls,
    ...
    def test_bad_ban_length(self):
        ...
        self.assertListEqual(
            [
                call(
                    "value for ban_length is invalid. invalid literal for int() with base 10: ''"
                ),
            ],
            self.error_mock.mock_calls,
        )
...
class Test_Tk_default_config(B3TestCase):
    ...
    def test_messages(self):
        self.assertEqual(
            "^7team damage over limit", self.p.config.get("messages", "ban")
        )
        self.assertEqual(
            "^7$vname^7 has forgiven $aname [^3$points^7]",
            self.p.config.get("messages", "forgive"),
        )
        self.assertEqual(
            "^7$vname^7 has a ^1grudge ^7against $aname [^3$points^7]",
            self.p.config.get("messages", "grudged"),
        )
        self.assertEqual(
            "^7$vname^7 has forgiven $attackers",
            self.p.config.get("messages", "forgive_many"),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            "^7no one to forgive", self.p.config.get("messages", "no_forgive")
        )
        self.assertEqual("^7Forgive who? %s", self.p.config.get("messages", "players"))
        self.assertEqual(
            "^7$name^7 has ^3$points^7 TK points",
            self.p.config.get("messages", "forgive_info"),
        )
        self.assertEqual(
            "^7$name^7 cleared of ^3$points^7 TK points",
            self.p.config.get("messages", "forgive_clear"),
        )
        self.assertEqual(
            "^3Do not attack teammates, ^1Attacked: ^7$vname ^7[^3$points^7]",
    ...
    def test__default_messages(self):
        ...
        for conf_message_id, conf_message in conf_items:
            if conf_message_id not in self.p._default_messages:
                self.fail(
                    "%s should be added to the _default_messages dict" % conf_message_id
                )
            if conf_message != self.p._default_messages[conf_message_id]:
                self.fail(
                    "default message in the _default_messages dict for %s does not match the message from the config file"
                    % conf_message_id
                )
