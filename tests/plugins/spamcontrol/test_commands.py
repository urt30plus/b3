...
class Test_commands(SpamcontrolTestCase):
    def setUp(self):
        ...
        with open(
            b3.functions.getAbsolutePath("@b3/conf/plugin_spamcontrol.ini")
        ) as default_conf:
            self.init_plugin(default_conf.read())

        self.joe = FakeClient(
            self.console, name="Joe", guid="zaerezarezar", groupBits=1
        )
        self.joe.connects("1")

        self.superadmin = FakeClient(
            self.console, name="Superadmin", guid="superadmin_guid", groupBits=128
        )
        self.superadmin.connects("2")

    def test_cmd_spamins(self):
        # GIVEN
        when(self.p).getTime().thenReturn(0).thenReturn(3).thenReturn(4).thenReturn(
            4
        ).thenReturn(500)
        ...
        self.assertListEqual(
            ["Joe currently has 9 spamins, peak was 9"], self.superadmin.message_history
        )  # 4s
        # WHEN
        self.superadmin.clearMessageHistory()
        self.superadmin.says("!spamins joe")
        self.assertListEqual(
            ["Joe currently has 0 spamins, peak was 9"], self.superadmin.message_history
        )  # 500s
    ...
    def test_cmd_spamins_lowercase(self):
        ...
        # THEN
        self.assertListEqual(
            ["Mike currently has 0 spamins, peak was 0"],
    ...
    def test_cmd_spamins_unknown_player(self):
        ...
        # THEN
        self.assertListEqual(
            ["No players found matching nobody"], self.superadmin.message_history
        )
    ...
    def test_cmd_spamins_no_argument(self):
        ...
        # THEN
        self.assertListEqual(
            ["You need to be in group Moderator to use !spamins"],
