...
class Test_default_config(Welcome_functional_test):
    ...
    def test_messages_greeting(self):
        self.assertEqual(
            "^7$name^7 joined: $greeting", self.conf.get("messages", "greeting")
        )

    def test_messages_greeting_empty(self):
        self.assertEqual(
            "^7You have no greeting set", self.conf.get("messages", "greeting_empty")
        )

    def test_messages_greeting_yours(self):
        self.assertEqual(
            "^7Your greeting is %s", self.conf.get("messages", "greeting_yours")
        )
    ...
    def test_messages_greeting_changed(self):
        self.assertEqual(
            "^7Greeting changed to: %s", self.conf.get("messages", "greeting_changed")
        )

    def test_messages_greeting_cleared(self):
        self.assertEqual(
            "^7Greeting cleared", self.conf.get("messages", "greeting_cleared")
        )
...
class Test_config_flags(Welcome_functional_test):
    ...
    def test_partly_set(self):
        ...
        self.assertEqual(
            F_FIRST | F_USER | F_ANNOUNCE_FIRST | F_CUSTOM_GREETING,
            self.p._welcomeFlags,
        )
